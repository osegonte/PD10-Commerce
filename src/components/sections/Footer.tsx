...
const SOCIALS = [
  {
    label: "TikTok",
    href: "https://www.tiktok.com/@martins_vincent?_r=1&_t=ZS-94AgN8j0J9B",
    icon: (
  ...
  {
    label: "YouTube",
    href: "https://youtube.com/@verydarkblackman?si=EpMlqHTEXxcxg7C2",
    icon: (
...
const accordionSections = [
  ...
  },
  {
    title: "Delivery",
    links: [
      { label: "3–7 Days Delivery", href: "/shipping-returns" },
      { label: "Pickup Available",  href: "/contact"          },
  ...
  return (
    <div className="border-t border-white/10">
      <button onClick={() => setOpen(!open)} className="w-full flex items-center justify-between py-4 text-left">
        ...
        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.2} stroke="currentColor" className="w-4 h-4 text-white/40 transition-transform duration-300" style={{ transform: open ? "rotate(180deg)" : "rotate(0deg)" }}>
      ...
      <div style={{ maxHeight: open ? "300px" : "0px", overflow: "hidden", transition: "max-height 0.35s ease" }}>
        <div className="pb-4 flex flex-col gap-3">
          {links.map((link) => (
            <Link key={link.label} href={link.href} className="text-[13px] text-white/50 hover:text-white transition-colors duration-200 font-light">
              {link.label}
            </Link>
          ))}
        </div>
    ...
    <footer className="bg-[#0a0a0a] border-t border-[#0a0a0a]">
      <div className="mx-8 sm:mx-14 lg:mx-20 py-14 sm:py-16">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-12 sm:gap-16">
          ...
          {/* ── Left: Brand + trust + contact + socials ── */}
          <div>
            <p className="text-white text-[22px] tracking-[0.15em] font-light mb-3" style={{ fontFamily: "'Helvetica Neue', Helvetica, Arial, sans-serif" }}>
              RATELS
            </p>
            <p className="text-white/50 text-[12px] leading-[1.8] font-light max-w-[280px] mb-8">
              Verified Devices. No Scams. Direct Deals.<br />Official store for trusted gadgets.
            </p>

            {/* Trust badges */}
            <div className="flex flex-col gap-2 mb-8">
              {["Verified Source", "Secure Checkout", "Delivery Tracking", "Official RATELS Channel"].map((item) => (
                <div key={item} className="flex items-center gap-2">
                  <span className="text-white/40 text-[11px]">✔</span>
                  <span className="text-white/40 text-[11px] tracking-[0.08em]">{item}</span>
                </div>
              ))}
            </div>
            ...
            <div className="space-y-2 mb-8">
              <a href="mailto:ratelsstore@gmail.com" className="flex items-center gap-2 text-white/40 hover:text-white transition-colors duration-300 text-[12px]">
                ...
                ratelsstore@gmail.com
              </a>
              <a href="tel:+2348000000000" className="flex items-center gap-2 text-white/40 hover:text-white transition-colors duration-300 text-[12px]">
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.2} stroke="currentColor" className="w-4 h-4 shrink-0">
        ...
        <div className="mt-14 pt-6 border-t border-white/10 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
          <p className="text-[11px] text-white/30 tracking-[0.08em]">
            © {new Date().getFullYear()} RATELS Store. All rights reserved.
          </p>
          <p className="text-[11px] text-white/30 tracking-[0.08em]">
            Made by{" "}
            <a href="https://osegonte.dev" target="_blank" rel="noopener noreferrer" className="hover:text-white transition-colors duration-200">
              Osegonte.webdev
            </a>
          </p>
        </div>
